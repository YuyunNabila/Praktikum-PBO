...
public class ReuseMarket {
    ...
    static void login() {
        ...
        for (Anggota a : daftarAnggota) {
            if (a.id.equals(username) && a.password.equals(password)) {
    ...
    static void hapusAnggota() {
        ...
        if (pilihan == 0) {
            ...
            System.out.println("\nAnggota " + anggotaDipilih.nama + " berhasil dihapus.");
        } else {
    ...
    static void lihatBarangPersetujuan() {
        ...
        for (Barang b : daftarBarang) {
            if (b.butuhPersetujuan) {
                barangMenungguPersetujuan.add(b);
            }
        }
        ...
        if (pilihan == 0) {
            ...
            if (opsi == 1) {
                barangDipilih.butuhPersetujuan = false;
                System.out.println("Lelang untuk barang " + barangDipilih.nama + " telah disetujui.");
            } else if (opsi == 2) {
                barangDipilih.butuhPersetujuan = false;
                barangDipilih.sudahDilelang = false;
                System.out.println("Lelang untuk barang " + barangDipilih.nama + " ditolak.");
            } else {
    ...
    static void lihatBarangVerifikasi() {
        ...
        for (Barang b : daftarBarang) {
            if (b.butuhVerifikasiPembayaran) {
        ...
        for (int i = 0; i < barangMenungguVerifikasi.size(); i++) {
            ...
            System.out.printf("%-5d %-20s Rp%,12d %-20s%n", 
                (i + 1), b.nama, b.hargaTertinggi, b.penawarTertinggi);
        }
        ...
        if (pilihan == 0) {
            ...
            if (opsi == 1) {
                barangDipilih.butuhVerifikasiPembayaran = false;
                System.out.println("Pembayaran untuk barang " + barangDipilih.nama + " telah diverifikasi.");
                daftarTransaksi.add(new Transaksi(barangDipilih, barangDipilih.penawarTertinggi, barangDipilih.hargaTertinggi, new Date()));
            } else if (opsi == 2) {
                System.out.println("Pembayaran untuk barang " + barangDipilih.nama + " ditolak.");
                barangDipilih.butuhVerifikasiPembayaran = false;
                barangDipilih.penawarTertinggi = "Belum ada bid";
                barangDipilih.hargaTertinggi = barangDipilih.hargaAwal;
                barangDipilih.jumlahBid = 0;
            } else {
                System.out.println("Opsi tidak valid.");
            }
        } else {
    ...
    static void tambahBarang() {
        ...
        daftarBarang.add(new Barang(id, nama, jumlah, kategori, hargaAwal, currentUser, false, false, false));
        System.out.println("\n====================================");
    ...
    static void ajukanLelang() {
        ...
        System.out.println("================================================");
        System.out.println("Daftar Barang Milik Anda yang Belum Dilelang:");
        ArrayList<Barang> barangBelumDilelang = new ArrayList<>();
        for (Barang b : daftarBarang) {
            if (b.pemilik != null && b.pemilik.equals(currentUser) && !b.sudahDilelang) {
        ...
        for (int i = 0; i < barangBelumDilelang.size(); i++) {
            ...
            System.out.printf("%-5d %-15s %-20s %-15s Rp%,12d%n", 
                (i + 1), b.id, b.nama, b.kategori, b.hargaAwal);
        }
        System.out.println("------------------------------------------------------------");
        ...
        boolean found = false;
        for (Barang b : daftarBarang) {
            if (b.id.equals(idBarang) && b.pemilik != null && b.pemilik.equals(currentUser) && !b.sudahDilelang) {
                b.butuhPersetujuan = true;
                b.sudahDilelang = true;
                System.out.println("\n====================================");
                System.out.println("Barang berhasil diajukan untuk dilelang!");
                System.out.println("ID Barang: " + b.id);
        ...
        if (!found) {
            System.out.println("\nError: ID Barang tidak ditemukan atau sudah dilelang.");
        }
    }
    ...
    static void lihatStatusLelang() {
        ...
        boolean found = false;
        for (Barang b : daftarBarang) {
            if (b.pemilik != null && b.pemilik.equals(currentUser) && b.sudahDilelang && !b.butuhPersetujuan) {
    ...
    static void lihatBarangMenungguPersetujuan() {
        ...
        boolean found = false;
        for (Barang b : daftarBarang) {
            if (b.pemilik != null && b.pemilik.equals(currentUser) && b.butuhPersetujuan) {
                b.tampilkanInfo();
                found = true;
            }
        }
    ...
    public static void lihatBarangSudahDilelang() {
        ...
        if (pilihan == 0) {
            ...
            if (barangDipilih.jumlahBid == 0 && System.currentTimeMillis() > barangDipilih.waktuBerakhir) {
                ...
                if (opsi == 1) {
                    barangDipilih.butuhPersetujuan = true;
                    barangDipilih.sudahDilelang = false;
                    barangDipilih.waktuBerakhir = System.currentTimeMillis() + (7 * 24 * 60 * 60 * 1000); // Reset waktu lelang
            ...
            } else if (barangDipilih.jumlahBid > 0 && System.currentTimeMillis() > barangDipilih.waktuBerakhir) {
                System.out.println("\nBarang ini sudah dilelang dan memiliki penawar.");
                System.out.println("Penawar Tertinggi: " + barangDipilih.penawarTertinggi + 
                                   " | Harga Tertinggi: " + barangDipilih.hargaTertinggi);
            }
        } else {
    ...
    public static void lihatSemuaBarangLelang() {
        ...
        for (Barang b : daftarBarang) {
            if (b.sudahDilelang && !b.butuhPersetujuan) {
                barangLelang.add(b);
            }
        }
        ...
        for (Barang b : barangLelang) {
            System.out.printf("%-10s %-20s %-15s Rp%,12d%n", 
                b.id, b.nama, b.kategori, b.hargaAwal);
        }
        System.out.println("------------------------------------------------------------");
    }
    ...
    public static void lihatBarangLelang() {
        ...
        switch (sortOption) {
            case 1:
                break; 
            case 2:
                barangLelang.sort(Comparator.comparingInt(b -> b.hargaAwal));
                break;
            case 3:
                barangLelang.sort((b1, b2) -> Integer.compare(b2.hargaAwal, b1.hargaAwal));
                break;
            case 4:
                barangLelang.removeIf(b -> !b.kategori.equalsIgnoreCase("Baru"));
                break;
            case 5:
                barangLelang.removeIf(b -> !b.kategori.equalsIgnoreCase("Bekas"));
                break;
            default:
    ...
    public static void lakukanPenawaran() {
        ...
        ArrayList<Barang> barangLelang = new ArrayList<>();
        for (Barang b : daftarBarang) {
            if (!b.pemilik.equals(currentUser) && b.sudahDilelang && !b.butuhPersetujuan) {
                barangLelang.add(b);
            }
        }
        if (barangLelang.isEmpty()) {
        ...
        for (Barang b : barangLelang) {
            System.out.printf("%-10s %-20s %-15s Rp%,12d%n", 
                b.id, b.nama, b.kategori, b.hargaTertinggi);
        }
        System.out.println("------------------------------------------------------------");
        ...
        boolean found = false;
        for (Barang b : barangLelang) {
            if (b.id.equals(idBarang)) {
                System.out.println("\nBarang ditemukan!");
                System.out.print("Masukkan jumlah bid Anda (minimal " + (b.hargaTertinggi + 1) + "): ");
                int bid = scanner.nextInt();
                scanner.nextLine();
                if (bid > b.hargaTertinggi && bid >= b.hargaAwal) {
                    b.hargaTertinggi = bid;
                    b.penawarTertinggi = currentUser;
                    b.jumlahBid++;
    ...
    public static void aktivitasLelang() {
        ...
        for (Barang b : daftarBarang) {
            if (b.penawarTertinggi.equals(currentUser)) {
                barangMenang.add(b);
            }
        }
        ...
        if (pilihan == 0) {
            ...
            System.out.println("\n=== Detail Barang ===");
            barangDipilih.tampilkanInfoLelang();
            if (System.currentTimeMillis() > barangDipilih.waktuBerakhir) {
                if (barangDipilih.butuhVerifikasiPembayaran) {
                    System.out.println("\nStatus: Menunggu Verifikasi Transaksi");
                } else if (!barangDipilih.butuhVerifikasiPembayaran && barangDipilih.sudahDilelang) {
                    ...
                    scanner.nextLine();
                    if (opsi == 1) {
                        barangDipilih.butuhVerifikasiPembayaran = true;
                        System.out.println("\nPermintaan verifikasi transaksi telah diajukan untuk barang ID: " + barangDipilih.id);
                        daftarTransaksi.add(new Transaksi(barangDipilih, currentUser, barangDipilih.hargaTertinggi, new Date()));
                    }
                }
            } else {
    ...
    public static void lihatRiwayatTransaksi() {
        ...
        for (Transaksi t : daftarTransaksi) {
            if (t.penawar.equals(currentUser) && !t.barang.butuhVerifikasiPembayaran && daftarBarang.contains(t.barang)) {
        ...
        System.out.println("------------------------------------------------------------");
        System.out.printf("%-5s %-20s %-15s %15s %-15s%n", "No", "Barang", "Penjual", "Harga (Rp)", "Tanggal");
        System.out.println("------------------------------------------------------------");
        for (int i = 0; i < riwayatTransaksiUser.size(); i++) {
            Transaksi t = riwayatTransaksiUser.get(i);
            System.out.printf("%-5d %-20s %-15s Rp%,12d %-15s%n", 
                (i + 1), t.barang.nama, t.barang.pemilik, t.harga, t.tanggal);
        }
        System.out.println("------------------------------------------------------------");
    }
}
